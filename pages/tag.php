<?php 
    ...
    // Znajdowanie czy wpisany tag istnieje
    $tag_id = DB::query('SELECT * FROM tags WHERE slug=:slug',["slug"=>$tag]);
    // Nie znaleziono
    if(!count($tag_id)) {
        include_once('404.php');
        exit;
    }
    $tag_name = $tag_id[0]['name'];
    $tag_id = $tag_id[0]['id'];

    // Pobieranie ilości tagów
    $how_much_posts = DB::query('SELECT COUNT(id) AS items FROM posts WHERE JSON_CONTAINS(keywords,'.$tag_id.')');
    ...
    $posts = DB::query('SELECT * FROM posts WHERE JSON_CONTAINS(keywords,'.$tag_id.') AND accepted=1 LIMIT '.$start_from.','.$per_page);
?>
...
<head>
    ...
    <title><?php echo $tag_name ?> - <?php echo DB::get_settings('website_title') ?></title>
